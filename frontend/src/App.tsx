  ...
  return <div className="app-shell" dir={direction}>
    ...
    <div className="workspace phase2-workspace">
      <aside className="sidebar">
        <section className="phase-card"><span className="eyebrow">Phase 4 · P4.5</span><h2>{locale==='ar'?'المسافة الجيوديسية WGS84':'WGS84 geodesic inspection'}</h2><p>{locale==='ar'?'حدد نقطتين من العرض المرجعي أو من نتائج البحث، ثم احسب المسافة والاتجاهات باستخدام محرك WGS84 الخلفي.':'Capture two reference points from the globe or search results, then calculate distance and bearings using the backend WGS84 engine.'}</p></section>
    ...
    <footer className="statusbar"><span>Phase 4 P4.5 in progress</span><span>WGS84-0.4.0 reference</span><span>Backend geodesic inverse · REFERENCE_RESULT</span><span>No Phase 5 synchronization</span></footer>
  </div>;
}
