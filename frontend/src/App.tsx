  ...
  return <div className="app-shell" dir={direction}>
    ...
    <div className="workspace phase2-workspace">
      ...
      <main className="phase2-main">
        <section className="reference-workspace"><ReferenceGlobe capabilities={capabilities} locale={locale} layers={globeLayers} layerPlaces={globePlaces} focusPoint={selectedPlace?{latitude:selectedPlace.latitude,longitude:selectedPlace.longitude}:null} focusLabel={selectedPlaceName} onPoint={(point)=>{setSelection({model:'wgs84',point});setSelectedPlace(current=>current&&Math.abs(current.latitude-point.latitude)<1e-9&&Math.abs(current.longitude-point.longitude)<1e-9?current:null);}}/></section>
    ...
    <footer className="statusbar"><span>{RELEASE_NAME} · {locale==='ar'?'المرحلة الرابعة بانتظار الاعتماد':'Phase 4 awaiting acceptance'}</span><span>WGS84-0.4.0 reference</span><span>Bundled countries + cached Phase 3 layers</span></footer>
  </div>;
}
