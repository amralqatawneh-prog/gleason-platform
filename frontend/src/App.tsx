...
import { GeographicSearchPanel } from './search/GeographicSearchPanel';
  ...
  return <div className="app-shell" dir={direction}>
    ...
    <div className="workspace phase2-workspace">
      <aside className="sidebar">
        <section className="phase-card"><span className="eyebrow">Phase 3 · v0.3.0</span><h2>{locale==='ar'?'طبقة البيانات والبحث':'Data & search layer'}</h2><p>{locale==='ar'?'PostGIS + بحث موحد + عقود حزم Offline، بدون إدخال بيانات إنتاجية مصطنعة.':'PostGIS + unified search + offline pack contracts, without fabricated production data.'}</p></section>
      ...
      <main className="phase2-main"><GeographicSearchPanel locale={locale} online={online && serverState==='connected'} /><div className="projection-grid"><ProjectionMap model="gleason" locale={locale} onPoint={handlePoint}/><ProjectionMap model="ae" locale={locale} onPoint={handlePoint}/></div><SourceViewer locale={locale}/></main>
    ...
    <footer className="statusbar"><span>Phase 3 geography/search foundation</span><span>GH-0.2.0 historical reconstruction</span><span>AE-0.2.0 independent reference</span><span>WGS84 globe: Phase 4</span></footer>
  </div>;
}
