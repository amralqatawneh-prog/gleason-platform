...
export type SelectionAction = { type: 'place'; place: PlaceSelection }
  ...
  | { type: 'restore'; selection: GeographicSelection };
export const INITIAL_SELECTION_STATE: SelectionState = Object.freeze({ selection: null, revision: 0 });
/** Rendering a marker never dispatches an action. Restore rehydrates state without pretending a user action occurred. */
export function selectionReducer(state: SelectionState, action: SelectionAction): SelectionState {
  if(action.type==='restore') return Object.freeze({selection:action.selection,revision:state.revision});
