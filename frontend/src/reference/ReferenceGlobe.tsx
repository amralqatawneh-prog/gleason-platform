...
export function ReferenceGlobe({ capabilities, locale, onPoint, focusPoint, focusLabel }: Props) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      gl.uniform1f(sxUniform, sx);
      gl.uniform1f(syUniform, sy);
  ...
  const choose = (point: ReferenceGeoPoint) => {
    setSelected(point);
    onPoint?.(point);
  };
  ...
  if (mode === 'fallback2d') {
    return <section className="reference-card" data-mode="fallback2d">
      ...
      <div className="reference-fallback" role="img" aria-label="WGS84 2D fallback"><svg viewBox="0 0 360 180" onClick={(event) => {
        const rect = event.currentTarget.getBoundingClientRect();
        choose({ latitude: 90 - ((event.clientY - rect.top) / rect.height) * 180, longitude: ((event.clientX - rect.left) / rect.width) * 360 - 180 });
      }}><rect width="360" height="180" rx="8"/>{[-120,-60,0,60,120].map((x)=><line key={`v${x}`} x1={x+180} x2={x+180} y1="0" y2="180"/>)}{[-60,-30,0,30,60].map((y)=><line key={`h${y}`} x1="0" x2="360" y1={90-y} y2={90-y}/>)}{focusPoint&&<circle cx={(focusPoint.longitude+180)} cy={(90-focusPoint.latitude)} r="4" className="reference-focus-marker"/>}</svg></div>
      <ReferenceReadout locale={locale} point={selected} mode="2D fallback" label={focusLabel}/>
    </section>;
  }
  ...
  return <section className="reference-card" data-mode="webgl3d">
    ...
    <ReferenceReadout locale={locale} point={selected} mode="WebGL2 3D" label={focusLabel}/>
  </section>;
}

function ReferenceReadout({locale,point,mode,label}:{locale:'ar'|'en';point:ReferenceGeoPoint;mode:string;label?:string}) {
  return <div className="reference-readout"><span>{locale==='ar'?'الوضع':'Mode'}: {mode}</span>{label&&<span>{locale==='ar'?'المكان':'Place'}: {label}</span>}<span>Lat {point.latitude.toFixed(6)}°</span><span>Lon {point.longitude.toFixed(6)}°</span><span>WGS 84</span></div>;
}
